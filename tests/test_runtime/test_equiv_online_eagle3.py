...
class TestEquivOnlineEagle3(unittest.TestCase):
    def test_equiv_online_eagle3(self):
        ...
        draft = AutoEagle3DraftModel.from_config(
            ...
            attention_backend="flex_attention", torch_dtype=torch.bfloat16,
        ).cuda()
        draft.load_vocab_mapping(vocab_path)
        draft.freeze_embedding()
        eagle3_model = OnlineEagle3Model(draft, length=TTT, attention_backend="flex_attention").cuda()
        eagle3_model.eval()
        ...
        def old_loss():
            ...
            plosses, _, _, _, _, _, _ = eagle3_model(
                ...
                loss_mask=d.loss_mask, target=d.target, hidden_states=d.hidden_states,
            )
        ...
        def new_loss():
            ...
            ctrl.ingest_prompts([{"payload": {"input_ids": ids, "loss_mask": [1] * SEQ}}])
            ...
            capture = CaptureConfig.from_strategy(
                ...
                target_repr="logits", target_hidden_size=H, target_vocab_size=V,
            )
            ...
            loader = FeatureDataLoader(
                store, ctrl.sample_queue, batch_size=1, collate_fn=cat_collate,
            )
        ...
        old = old_loss()
        new = new_loss()
        self.assertAlmostEqual(old, new, places=3, msg=f"online loss: old={old} new={new}")
