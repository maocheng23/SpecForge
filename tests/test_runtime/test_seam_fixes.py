...
def _ref(i):
    return SampleRef(
        sample_id=f"s{i}", run_id="r", source_task_id=None,
        feature_store_uri=f"mem://x/s{i}", feature_keys={}, feature_specs={},
        strategy="eagle3",
    )
...
class TestDurableAckTransaction(unittest.TestCase):
    def test_ack_records_durable_marker(self):
        ...
        ctrl.ack_train_refs("t0", [r.sample_id for r in leased], global_step=7,
...
class TestParallelConfigHandles(unittest.TestCase):
    def test_carries_all_parallel_fields(self):
        pc = ParallelConfig()
        for f in ("tp_group", "sp_ulysses_group", "sp_ring_group", "draft_sp_group",
                  "device_mesh", "dp_group", "draft_dp_group"):
...
class TestDFlashSharesLifecycle(unittest.TestCase):
    ...
    def test_dflash_validate_batch_rejects_missing(self):
        ...
        bad = TrainBatch(sample_ids=["s"], strategy="dflash",
