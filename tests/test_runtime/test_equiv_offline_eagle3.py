...
def _write_target_head_dir(d, hidden, vocab):
    ...
    with open(os.path.join(d, "model.safetensors.index.json"), "w") as f:
        json.dump({"metadata": {}, "weight_map": {"lm_head.weight": "model.safetensors"}}, f)
        ...
        torch.save(
            {
                ...
                "hidden_state": torch.randn(1, seq, hidden, generator=g).to(torch.bfloat16),
                "aux_hidden_state": torch.randn(1, seq, 3 * hidden, generator=g).to(torch.bfloat16),
            },
...
class TestEquivOfflineEagle3(unittest.TestCase):
    def test_equiv_offline_eagle3(self):
        ...
        from specforge.data import build_offline_eagle3_dataset
        ...
        def old_path_losses():
            ...
            for start in range(0, N, BS):
                ...
                losses.append(sum(w[i] * plosses[i] for i in range(len(plosses))).item())
            return losses
