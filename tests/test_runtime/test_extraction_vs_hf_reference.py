...
class TestExtractionVsHFReference(unittest.TestCase):
    def test_extraction_vs_hf_reference(self):
        ...
        target, target_dir, aux_ids = fx.build_hf_target(workdir, hidden=H, layers=8, vocab=V)
        ...
        task = PromptTask(
            task_id="t0", run_id="r", source_id="s",
            payload={"input_ids": ids, "loss_mask": [1] * SEQ}, max_length=SEQ,
        )
        ...
        for k, layer_id in enumerate(aux_ids):
            ...
            torch.testing.assert_close(got, ref_layer, rtol=RTOL, atol=ATOL,
        ...
        torch.testing.assert_close(
            adapter_target[:, : SEQ - 1, :], ref_logits[:, 1:SEQ, :],
            rtol=RTOL, atol=ATOL, msg="target logits drift vs direct lm_head",
        )
    ...
    def test_capture_layer_mismatch_fails(self):
        ...
        with self.assertRaises(CaptureMismatchError):
            verify_capture(tensors, cap, sample_id="x", recorded_aux_layer_ids=(1, 3, 5))
