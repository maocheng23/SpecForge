...
class TestCheckpointResume(unittest.TestCase):
    def test_checkpoint_resume(self):
        ...
        def build_model():
            ...
            dm.freeze_embedding()
            return OnlineEagle3Model(dm, length=TTT, attention_backend="flex_attention").cuda()
            ...
            for s in range(0, N, BS):
                ...
                out.append(
                    TrainBatch(
                        ...
                        strategy="eagle3", tensors=dict(data),
        ...
        model = build_model()
        opt = BF16Optimizer(model.draft_model, lr=1e-3, max_grad_norm=0.5,
        ...
        ctrl = TrainerController(core, run_id="r", output_dir=out_dir, max_steps=3, num_epochs=2)
        ...
        for k, v in trained.items():
            self.assertTrue(torch.equal(v.cpu(), fresh_sd[k].cpu()), msg=f"weight {k} mismatch")
