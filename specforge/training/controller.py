...
class TrainerCore:
    ...
    def train_step(
        ...
        if _P:
            ...
            if acc is None:
                acc = {"fwd": 0.0, "bwd": 0.0, "opt": 0.0, "n": 0,
                       "B": 0, "padT": 0, "useful": 0.0}
                self._prof2 = acc
            acc["fwd"] += _a1 - _a0
            acc["bwd"] += _a2 - _a1
            acc["opt"] += _a3 - _a2
...
class TrainerController:
    ...
    def fit(
        ...
        _TPROF = int(os.environ.get("PROFILE_TORCH", "0"))  # active optimizer steps to profile; 0=off
        _tprof_warmup = int(os.environ.get("PROFILE_TORCH_WARMUP", "40"))
        _tprof_rank0 = (not torch.distributed.is_initialized()) or torch.distributed.get_rank() == 0
