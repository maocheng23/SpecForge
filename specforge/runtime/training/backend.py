    ...
    def from_distributed(
        ...
        *,
        ...
        if not dist.is_initialized():
            return cls(
                world_size=1, tp_size=tp_size,
                sp_ulysses_size=sp_ulysses_size, sp_ring_size=sp_ring_size,
                sharding_strategy=sharding_strategy, param_dtype=param_dtype,
            )
...
class FSDPTrainingBackend(TrainingBackend):
    ...
    def step(self) -> Optional[torch.Tensor]:
        ...
        if self.optimizer is None:
            raise RuntimeError("FSDPTrainingBackend.step called before optimizer is set")
